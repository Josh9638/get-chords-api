...
from .models import SongGenre, SongCategory, SongAlbum, SongMusician, SongChords, SongPlaylist, Chords
from django.contrib.auth.models import User
from .serializers import SongGenreSerializer, SongCategorySerializer, SongAlbumSerializer, SongMusicianSerializer, SongChordsSerializer, SongPlaylistSerializer, ChordsSerializer
...
class ChordAnalizerApiView(APIView):
    ...
    def post(self, request):
        ...
        if chords_of_song.is_valid():
            ...
            #actualizamos el registro de SongChords
            chords_songs_updated = update_chords(chords_of_song.data['id'], audio_name, list(chords_gen))
...
chord_analizer = ChordAnalizerApiView.as_view()


def update_chords(id, audio_name, chords):
    with connection.cursor() as cursor:
        cursor.execute("UPDATE song_chords_lite SET audio_file = %s, chords = %s WHERE id = %s", [audio_name, str(chords), id])
    ...
    #song_chords.chords = chords
    #song_chords.sample_rate = sample_rate
    song_chords = Chords.objects.get(id=id)

    serializer_class = ChordsSerializer(song_chords)
